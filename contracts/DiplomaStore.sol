...
contract DiplomaStore {
    ...
    function addDiploma(string memory _cnp, string memory _city, string memory _emailStudent, string memory _degree, string memory _hashValue) public stopInEmergency{
        ...
        if(keccak256(abi.encodePacked(addres)) != keccak256(abi.encodePacked(toString(msg.sender)))){
            stopped = true;
        }
